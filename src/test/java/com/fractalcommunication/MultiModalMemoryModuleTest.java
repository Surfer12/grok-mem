...
public class MultiModalMemoryModuleTest {
  ...
  public void testLoadUserProfile_NewUser() throws FCFException {
    ...
    assertEquals(
        "fractal",
        profile.getInteractionStyle().get("style"),
        "Default interaction style should be set.");
    assertEquals(
        1, profile.getPreferredAnchors().size(), "Default preferred anchors should be set.");
    assertEquals(
        "connection",
        profile.getPreferredAnchors().get(0),
        "Default preferred anchor should be 'connection'.");
  }
  ...
  public void testLoadUserProfile_ExistingUser() throws FCFException {
    ...
    assertSame(
        profile1, profile2, "Loading the same user ID should return the same profile instance.");
  }
  ...
  public void testSaveLongTermInteractionStyle() throws FCFException {
    ...
    assertEquals(
        "updated",
        profile.getInteractionStyle().get("style"),
        "Interaction style should be updated.");
    assertEquals(
        "therapeutic",
        profile.getInteractionStyle().get("mode"),
        "New interaction style entry should be added.");
  }
  ...
  public void testSaveSessionMemory() throws FCFException {
    ...
    assertEquals(
        1,
        metrics.get("activeSessions"),
        "Active sessions count should reflect saved session memory.");
  }
