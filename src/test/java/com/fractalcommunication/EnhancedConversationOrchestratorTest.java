...
public class EnhancedConversationOrchestratorTest {
  ...
  public void testRunConversation_SuccessfulExecution() throws FCFException {
    ...
    assertTrue(
        response.contains("Applying custom anchor"),
        "Response should contain anchor application output.");

    Map<String, Object> metrics = metricsLogger.getAllMetrics();
    assertEquals(
        1,
        metrics.get("ReflectionEngine_reflectionLength_count"),
        "Reflection metrics should be logged.");
    assertEquals(
        1,
        metrics.get("AnchorModule_selectedAnchor_count"),
        "Anchor selection metrics should be logged.");
    assertEquals(
        1,
        metrics.get("SynthesisModule_outputLength_count"),
        "Synthesis metrics should be logged.");
  }
  ...
  public void testGenerateExercise_SuccessfulExecution() throws FCFException {
    ...
    assertEquals(
        "Challenge: " + challenge, steps.get(0), "First step should be the challenge description.");

    Map<String, Object> metrics = metricsLogger.getAllMetrics();
    assertEquals(
        1,
        metrics.get("ExerciseEngine_exerciseStepsGenerated_count"),
        "Exercise generation metrics should be logged.");
  }
  ...
  public void testHandleInterruption() throws FCFException {
    ...
    assertEquals(
        1,
        metrics.get("Orchestrator_interruptionHandled_count"),
        "Interruption handling metrics should be logged.");
  }
  ...
  public void testHandleBranching() throws FCFException {
    ...
    assertEquals(
        1,
        metrics.get("Orchestrator_branchCreated_count"),
        "Branch creation metrics should be logged.");
  }
  ...
  public void testFallbackRoutine() throws FCFException {
    ...
    assertTrue(
        fallbackResponse.contains(userInput), "Fallback response should mention the user input.");
    assertTrue(
        fallbackResponse.contains("clarify"), "Fallback response should ask for clarification.");

    Map<String, Object> metrics = metricsLogger.getAllMetrics();
    assertEquals(
        1,
        metrics.get("Orchestrator_fallbackTriggered_count"),
        "Fallback metrics should be logged.");
  }
}
