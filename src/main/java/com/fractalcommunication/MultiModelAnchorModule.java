  ...
  public MultiModelAnchorModule(IMetricsLogger metricsLogger) {
    ...
    try {
      ...
      metricsLogger.logError(
          "AnchorModule", "Failed to initialize xAI selector: " + e.getMessage(), e);
    }
    ...
    try {
      ...
      metricsLogger.logError(
          "AnchorModule", "Failed to initialize OpenAI selector: " + e.getMessage(), e);
    }

    // Register default anchors
    registerAnchor(
        new CustomAnchor("Grounding", "Focus on breath or body to create a safe container."));
    ...
    registerAnchor(
        new CustomAnchor("Transformation", "Identify a mindful action to shift patterns."));
    ...
    if (defaultSelector != null) {
      for (IMLAnchorSelector selector : mlSelectors) {
        try {
          ...
          metricsLogger.logError(
              "AnchorModule",
              "ML selector " + selector.getModelName() + " failed: " + e.getMessage(),
              e);
        }
      }
    }
