...
public class BasicExerciseEngineTest {
  ...
  public void testGenerateSteps() throws FCFException {
    ...
    assertEquals(
        "Challenge: " + challenge,
        result.get(0),
        "First element should be the challenge description.");
    assertEquals(
        "Step 1: Grounding", result.get(1), "First step should match the first anchor name.");
    assertEquals(
        "Step 2: Openness", result.get(2), "Second step should match the second anchor name.");
  }
  ...
  public void testGetExerciseMetrics() {
    ...
    assertEquals(
        1, metrics.get("exercisesGenerated"), "Exercises generated count should be 1 in metrics.");
  }
}
