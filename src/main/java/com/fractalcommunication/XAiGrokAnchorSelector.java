...
public class XAiGrokAnchorSelector implements IMLAnchorSelector {
    ...
    public XAiGrokAnchorSelector() {
        this.httpClient = HttpClient.newBuilder()
            .connectTimeout(XAiConfig.getTimeout())
            .build();
    ...
    public String selectAnchorWithML(IConversationState state, Map<String, ITherapeuticAnchor> availableAnchors) throws FCFException {
        ...
        String prompt = String.format(
            ...
            anchorList,
            state.getUserInput(),
            state.getHistory().isEmpty() ? "No history yet." : String.join("; ", state.getHistory().subList(Math.max(0, state.getHistory().size() - 3), state.getHistory().size())),
        ...
        requestBody.put("messages", List.of(
            Map.of("role", "system", "content", "You are a helpful assistant for selecting therapeutic conversational strategies."),
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                throw new FCFException("xAI API returned non-200 status: " + response.statusCode() + ", body: " + response.body(), null);
            }
            ...
            String selectedAnchorName = responseText.split("\n")[0].trim().split(" ")[0].replace("'", "").replace("\"", "");
            ...
            successCount++;
            System.out.println("xAI selected anchor: " + selectedAnchorName + " with rationale: " + responseText);
